...
### M1-10 会话中心 🟨（2026-09-20 代码完成＋自验通过，待所有者 dev 验收）
...
- 待所有者 dev 验收：会话窗口全功能（筛选/搜索/排序/分页/导出/跳转）、面板上限与
  「查看更多会话」入口、报表页瘦身形态、深浅主题、多窗口主题联动
- ⚠️ 并行开发注意：本任务开发期间 lib.rs 岛显隐逻辑由所有者并行重构
  （peek_apply → island_transition 状态机），会话相关改动已在新结构上验证通过
- 依赖：M1-9（托盘菜单形态，已提交 8cb5248）

### M1-11 会话数据深利用：指标补全＋会话详情抽屉 🟨（2026-09-20 代码完成＋桩数据自验，待所有者 dev 验收）
...
- 待所有者 dev 验收：思考/首字指标、抽屉流水与时间线（hooks 启用后有真实事件）
- 依赖：M1-10
